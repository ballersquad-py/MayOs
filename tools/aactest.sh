...
ffmpeg -v error -y -f lavfi -i "$src" $enc -f adts $S/$name.aac || exit 1
ffmpeg -v error -y -i $S/$name.aac -f s16le $S/$name.ref.pcm
CH=$(ffprobe -v error -show_entries stream=channels -of csv=p=0 $S/$name.aac)
$(dirname "$0")/../libs/target/release/examples/aacdec $S/$name.aac $S/$name.out.pcm 2> $S/$name.log
